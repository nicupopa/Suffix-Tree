...
(define (longest-common-prefix w1 w2)
  (longest-prefix w1 w2 '())
  )

(define (longest-prefix w1 w2 acc)
  (if (or (null? w1) (null? w2))
      (list (reverse acc) w1 w2)
      (if (equal? (car w1) (car w2))
          (longest-prefix (cdr w1) (cdr w2) (cons (car w1) acc))
          (list (reverse acc) w1 w2)
          )
      )
  )
...
(define (longest-common-prefix-of-list words)
  (longest-helper words '())
  )

(define (longest-helper words acc)
  (if (or (null? words) (null? (cdr words)))
      (car acc)
      (longest-helper (cdr words) (longest-common-prefix (car words) (cadr words)))
      )
  )
...
(define (match-pattern-with-label st pattern)
  (if (get-ch-branch st (car pattern))
      (if (equal? pattern (car (longest-common-prefix pattern (get-branch-label (get-ch-branch st (car pattern))))))
          #t
          (if (equal? (get-branch-label (get-ch-branch st (car pattern))) (car (longest-common-prefix pattern (get-branch-label (get-ch-branch st (car pattern))))))
              (list (car (longest-common-prefix pattern (get-branch-label (get-ch-branch st (car pattern)))))
                    (cadr (longest-common-prefix pattern (get-branch-label (get-ch-branch st (car pattern)))))
