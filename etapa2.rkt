...
; folosiți funcționale.
  (define (suffixes->st labeling-func suffixes alphabet)
    (filter not-null?
            (map (lambda (ch)
                   (if (null? (filter not-null? (get-ch-words suffixes ch)))
                       '()
                       (cons (car (labeling-func (filter not-null? (get-ch-words suffixes ch))))
                             (suffixes->st labeling-func (filter not-null? (cdr (labeling-func (filter not-null? (get-ch-words suffixes ch))))) alphabet))))
                 alphabet))
    )

  (define (not-null? word)
    (not (null? word))
    )
  ...
  (lambda (labeling-func)
    (lambda (text)
      (suffixes->st labeling-func (get-suffixes (append text '(#\$)))
                                                    (sort (remove-duplicates (append text '(#\$))) char<?)))))
